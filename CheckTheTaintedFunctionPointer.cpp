...
VOID ReadMem(UINT32 insAddr, std::string insDis, UINT32 OperandCount,REG reg_r, UINT32 memOp, UINT32 sp)
{
  ...
  //std::cout  << "insDis:" << "insDis" << std::endl;
  //if (opCount != 2)
  //  return;
  //memOp=memOp%0x100000000;
  if (OperandCount != 2)
    return;
  for(i = addressTainted.begin(); i != addressTainted.end(); i++){
      if (addr == *i){
...
VOID spreadRegTaint(UINT32 insAddr, std::string insDis, UINT32 OperandCount,REG reg_r, REG reg_w)
{
  ...
  //if (OperandCount == 4)
	//std::cout  <<std::hex<<"[ "<<insAddr << " ]insDis:" << insDis << std::endl;
  if (REG_valid(reg_w)){
...
VOID followData(UINT32 insAddr, std::string insDis, REG reg)
{
  ...
  if (checkAlreadyRegTainted(reg)){
      std::cout << std::hex <<insAddr<< ":\t"  <<"[ERROR eax is tainted in :]" << " insDis:" << insDis << std::endl;
  }
}
